  ...
  return function isAnimationNameRec(node, variables) {
    ...
    if (node.type === 'Identifier' && variables && variables.has(node.name)) {
      ...
      if (variable != null) {
        ...
        return {
          message: 'All expressions in a template literal must be a `keyframes(...)` function call'
        };
      }
    }
    if (node.type === 'TemplateLiteral') {
      ...
      if (!node.quasis.every((quasi, index, {
        length
      }) => index === 0 || index === length - 1 ? quasi.value.raw === '' : quasi.value.raw === ', ')) {
        return {
    ...
    return {
      message: 'a `keyframes(...)` function call, a reference to it or a many such valid'
    };
  };
}